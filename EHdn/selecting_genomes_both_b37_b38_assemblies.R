...
df_platekey_both = main_clin_data %>%
  #filter(plate_key %in% l_platekey_both, genome_build %in% "GRCh38", grepl("HX", file_path)) %>%
  filter(plate_key %in% l_platekey_both, genome_build %in% "GRCh38") %>%
  select(plate_key, file_path)
...
selected = c()
for (i in 1:length(df_platekey_both$plate_key)){
  l_loci = df_platekey_both %>% filter(plate_key %in% df_platekey_both$plate_key[i])
  l_year = unlist(lapply(strsplit(l_loci$file_path, '/'), '[', 4))
  selected = rbind(selected, l_loci[pmatch(max(l_year),l_year),])
}
dim(selected)
...
# Include this to df_platekey_both
df_platekey_both = selected

length(unique(df_platekey_both$plate_key))
# 1664
length(unique(df_platekey_both$file_path))
# 1664

write.table(df_platekey_both,
            "~/Documents/STRs/ANALYSIS/EHdn/EHdn-v0.8.6/input/list_1664_genomes_both_b37_and_b38_in_GRCh38.csv",
            sep = ",",
            row.names = F,
            col.names = F,
            quote = F)
