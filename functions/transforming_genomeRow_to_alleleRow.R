...
# Load data
if (!file.exists(merged_table)){
  print("The file corresponding to the merged table does not exist")
  break
}
  
merged_data = read.csv(merged_table,
                       sep = '\t',
                       header = T,
                       stringsAsFactors = F)
dim(merged_data)
    ...
    for (j in 1:length(locus_data$chr)){
      ...
      for (k in 1:length(number_samp)){
        ...
        if (dim(to_include)[1] <= 0){
          ...
          colnames(to_include) = c("participant_id", "platekey", "rare_diseases_family_id", "specific_disease", "disease_group", 
                                   "disease_sub_group", "year_of_birth", "participant_phenotypic_sex", "biological_relationship_to_proband", 
                                   "affection_status", "family_group_type", "hpo_list", "panel_list", "programme", "genome_build", "population")
        }
