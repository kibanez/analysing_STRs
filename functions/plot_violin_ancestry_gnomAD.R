plot_violin_ancestry_gnomAD <- function(df_input, gene_name, gene_data_normal, gene_data_pathogenic, output_folder){
  ...
  df_gene = df_input %>% 
    ...
    select(allele, num_samples, superpopu)
  
  df_gene2 = data.frame()
  l_ancestries = unique(df_gene$superpopu)
  for(i in 1:length(l_ancestries)){
    aux_df = df_gene %>% filter(superpopu %in% l_ancestries[i])
    for(j in 1:length(aux_df$allele)){
      allele = aux_df$allele[j]
      repeat_size = aux_df$num_samples[j]
      superpopulation = aux_df$superpopu[j]
      
      new_line = c(allele, superpopulation)
  ...
  colnames(df_gene2) = c("repeat_size", "superpopulation")
  ...
  violin_plot = ggplot(df_gene2, aes(x = superpopulation, y=repeat_size, fill = superpopulation)) +
    geom_violin() +
    coord_flip() +
    xlab("Population") + 
