...
clin_data_merged = read.csv("./clin_data_merged_V1:V10.tsv",
                            sep = "\t",
                            stringsAsFactors = F,
                            header = T)
dim(clin_data_merged)
...
list_platekeys1 = clin_data_merged %>% group_by(participant_id) %>% summarise(list_platekeys1 = toString(unique(platekey))) %>% ungroup() %>% as.data.frame()
list_platekeys2 = clin_data_merged %>% group_by(participant_id) %>% summarise(list_platekeys2 = toString(unique(plate_key))) %>% ungroup() %>% as.data.frame()
...
# Remove old columns
drops <- c("normalised_specific_disease","panel_name","panel_version", "hpo_term", "hpo_id", "disease_group", "disease_sub_group", "platekey", "plate_key")
...
dedup_clin_data_merged = dedup_clin_data_merged %>%
  select(participant_id, list_platekeys1, list_platekeys2, rare_diseases_family_id, biological_relationship_to_proband, family_group_type, list_norm_disease, list_disease_group, list_disease_subgroup, list_hpos, list_hpos_id, participant_phenotypic_sex,
